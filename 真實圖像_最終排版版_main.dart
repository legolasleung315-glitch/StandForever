...
class _PhotoProcessorPageState extends State<PhotoProcessorPage> {
  ...
  List<Widget> _buildSettingsChildren() {
    return [
          ...
          if (_adjustmentMode == 0) ...[
            ...
            Slider(value: _jpegQuality.toDouble(), min: 60, max: 98, divisions: 38, onChanged: _isProcessing ? null : (v) => _setStateAndSave(() => _jpegQuality = v.round())),
            Text('輸出尺寸（寬度）：$_maxWidth px'),
            Slider(value: _maxWidth.toDouble().clamp(500, 4000), min: 500, max: 4000, divisions: 70, onChanged: _isProcessing ? null : (v) => _setStateAndSave(() => _maxWidth = v.round())),
          ] else ...[
            _precisionControl(title: 'JPG 品質', value: _jpegQuality, min: 60, max: 98, step: 1, unit: '', disabled: _isProcessing, onChanged: (v) => _setStateAndSave(() => _jpegQuality = v.round())),
            _precisionControl(title: '輸出尺寸', value: _maxWidth, min: 500, max: 4000, step: 50, unit: ' px', disabled: _isProcessing, onChanged: (v) => _setStateAndSave(() => _maxWidth = v.round())),
          ],
          SwitchListTile(
            contentPadding: EdgeInsets.zero,
  ...
  Widget _buildSettingsPage() {
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: _buildSettingsChildren()),
    );
  }
