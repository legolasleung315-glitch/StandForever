...
class _PhotoProcessorPageState extends State<PhotoProcessorPage> {
  ...
  final Saf _saf = Saf();
  String? _safOutputDirectoryUri;
  final Map<String, String> _safUriByCachePath = <String, String>{};
  ...
  Future<void> _loadSavedSettings() async {
    ...
    final sm = prefs.getInt('startMinute') ?? 35;
    final eh = prefs.getInt('endHour') ?? 8;
    final em = prefs.getInt('endMinute') ?? 55;
    setState(() {
      _prefs = prefs;
  ...
  /// 第一次使用時選擇輸出資料夾並保存真正的 SAF URI 授權。
  /// Android 上不再把 /storage/emulated/0/... 當成可直接寫入的路徑，
  /// 以避免 vivo / Android scoped storage 的 Operation not permitted。
  Future<String?> _chooseOutputDirectory() async {
    try {
      ...
      if (Platform.isAndroid) {
        final savedUri = prefs.getString('outputDirectorySafUri');
        final savedLabel = prefs.getString('outputDirectoryLabel');
        if (savedUri != null && savedUri.isNotEmpty) {
          final grants = await _saf.persistedPermissions();
          final stillGranted = grants.any((g) => g.uri == savedUri);
          if (stillGranted) {
            _safOutputDirectoryUri = savedUri;
            return savedLabel?.isNotEmpty == true ? savedLabel : '已授權輸出資料夾';
          }
          await prefs.remove('outputDirectorySafUri');
          await prefs.remove('outputDirectoryLabel');
        }
        ...
        _safOutputDirectoryUri = selected.uri;
        await prefs.setString('outputDirectorySafUri', selected.uri);
        await prefs.setString('outputDirectoryLabel', selected.name);
      ...
      if (savedDirectory != null && savedDirectory.isNotEmpty) {
        ...
        if (await savedDir.exists()) return savedDirectory;
  ...
  Future<void> _processImages() async {
    ...
    if (Platform.isAndroid) {
      final rootUri = _safOutputDirectoryUri;
      if (rootUri == null || rootUri.isEmpty) {
        _showError('輸出資料夾授權已失效，請重新選擇。');
        return;
      }
      var targetUri = rootUri;
      if (_createWorkFolder) {
        final folderDate = _useTodayDate ? DateTime.now() : _startDateTime;
        final folderName = '${DateFormat('yyyyMMdd').format(folderDate)}_工作記錄';
        final target = await _saf.mkdirp(rootUri, [folderName]);
        targetUri = target.uri;
        outputDirectory = '$baseDirectory/$folderName';
      }
      _safOutputDirectoryUri = targetUri;
    } else if (_createWorkFolder) {
      final folderDate = _useTodayDate ? DateTime.now() : _startDateTime;
      final folderName =
      ...
      for (int batchStart = 0; batchStart < total; batchStart += workerCount) {
        ...
        for (final result in results) {
          ...
          try {
            ...
            String safePath;
            if (Platform.isAndroid) {
              // 先寫入 App 自己可安全寫入的暫存目錄，再透過 SAF 複製到使用者授權的資料夾。
              final tempDir = Directory.systemTemp;
              safePath = '${tempDir.path}${Platform.pathSeparator}$fileName';
              final tempFile = File(safePath);
              await tempFile.writeAsBytes(outputBytes, flush: true);

              final targetUri = _safOutputDirectoryUri;
              if (targetUri == null || targetUri.isEmpty) {
                throw StateError('SAF 輸出資料夾授權不存在');
              }
              final saved = await _saf.pasteLocalFile(
                safePath,
                targetUri,
                fileName,
                'image/jpeg',
              ...
              _safUriByCachePath[safePath] = saved.uri;
            } else {
              final outPath =
                  '$outputDirectory${Platform.pathSeparator}$fileName';
              safePath = await _makeSafeOutputPath(outPath);
              ...
              if (_applyTime) {
                try {
                  ...
                  _errors.add('$name：圖片已輸出，但無法修改檔案修改時間：$e');
                }
              }
            }

            try {
              // 以本機暫存檔（或非 Android 的實際檔案）加入 Android 圖庫。
              await ImageGallerySaverPlus.saveFile(
                safePath,
                name: fileName.replaceFirst(RegExp(r'\.jpg$'), ''),
              );
            } catch (e) {
              _errors.add('$name：圖片已輸出，但加入 Android 圖庫失敗：$e');
            }
  ...
  Future<void> _prepareRenameSequences(String directory) async {
    ...
    if (Platform.isAndroid) {
      final uri = _safOutputDirectoryUri;
      if (uri == null || uri.isEmpty) return;
      final entries = await _saf.list(uri);
      for (final entry in entries) {
        if (entry.isDir) continue;
        _registerExistingOutputName(entry.name);
      }
      return;
    }
    ...
    await for (final entity in dir.list(followLinks: false)) {
      if (entity is File) _registerExistingOutputName(entity.uri.pathSegments.last);
    }
  }

  void _registerExistingOutputName(String name) {
    ...
    if (withDate != null) {
      ...
      _nextSequenceByKey[key] = math.max(_nextSequenceByKey[key] ?? 1, number + 1);
      return;
    }
    ...
    try {
      ...
      await SharePlus.instance.share(
        ShareParams(
          files: files,
          title: '真實圖像',
          text: files.length == 1 ? '真實圖像處理結果' : '真實圖像處理結果（${files.length} 張）',
        ),
      );
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _deleteResults() async {
    if (_resultFiles.isEmpty || !mounted) return;

    final count = _resultFiles.length;
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title: const Text('刪除處理結果？'),
        content: Text('確定要刪除剛才生成的 $count 張圖片嗎？\n\n刪除後無法復原。'),
        actions: [
          TextButton(
    ...
    int deleted = 0;
    int failed = 0;
    for (final path in List<String>.from(_resultFiles)) {
      try {
        if (Platform.isAndroid) {
          final uri = _safUriByCachePath[path];
          if (uri != null) await _saf.delete(uri);
        }
    ...
    if (!mounted) return;
    _resultFiles.clear();
    _safUriByCachePath.clear();

    Navigator.of(context).pop();
    final message = failed == 0
        ? '已刪除 $deleted 張圖片'
        : '已刪除 $deleted 張圖片，$failed 張刪除失敗';
