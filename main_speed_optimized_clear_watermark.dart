...
Future<Uint8List> _processImageInIsolate(
  ...
  if (filterMode == 1) {
    ...
    image = img.colorOffset(image, red: 3, green: 1, blue: -3);
  } else if (filterMode == 2) {
    // Note 2 reference look:
    // - lower contrast
    // - lower saturation
    // - visibly more neutral/gray
    // - only a very small amount of softening
    image = img.adjustColor(
      image,
      contrast: 0.86,
      saturation: 0.68,
    ...
    // Mix about 32% luminance into the color image. This is deliberately
    // stronger than a tiny saturation reduction so the Note 2 mode is
    // visibly different on both large and small outputs.
    const grayMix = 0.32;
...
void _drawWatermark(
  ...
  // A restrained shadow improves readability on pavement/sky without making
  // the timestamp look like a heavy outlined caption.
  final shadow = img.ColorRgba8(0, 0, 0, (alpha * 0.55).round());
  ...
  // Adaptive watermark rendering for arbitrary output widths.
  // The configured size is still derived from the real output width, but
  // small outputs get a dedicated crisp final-pixel treatment instead of
  // relying only on a tiny linear resize. This is important for 500-800 px
  // images, where JPEG quantisation can otherwise erase thin digit strokes.
  final bool tiny = targetHeight <= 18;
  final bool small = targetHeight <= 32;
  final int workScale = tiny ? 4 : (small ? 3 : 2);

  final enlarged = img.copyResize(
    overlay,
    width: math.max(1, targetWidth * workScale),
    height: math.max(1, targetHeight * workScale),
  ...
  var scaled = img.copyResize(
    ...
    width: targetWidth,
    height: targetHeight,
    // Linear is retained above 800-ish px; for tiny glyphs we use nearest
    // for the final pass so very thin date strokes are not averaged away.
    interpolation: tiny ? img.Interpolation.nearest : img.Interpolation.linear,
  );

  if (tiny) {
    // 1) Strengthen the alpha coverage of the actual glyph pixels.
    // 2) Add a very restrained 1-pixel dark separation around the final
    //    glyph, rather than enlarging the glyph itself. This survives JPEG
    //    compression much better against detailed backgrounds.
    final hardened = img.Image(
      width: scaled.width,
      height: scaled.height,
      numChannels: 4,
    );
    hardened.clear(img.ColorRgba8(0, 0, 0, 0));

    for (var yy = 0; yy < scaled.height; yy++) {
      for (var xx = 0; xx < scaled.width; xx++) {
        final p = scaled.getPixel(xx, yy);
        final a = p.a / 255.0;
        final boosted = ((a - 0.30) * 1.75 + 0.30).clamp(0.0, 1.0);
        p.a = (boosted * 255.0).round();
      }
    }

    // Build a compact shadow from the final glyph alpha. The shadow is kept
    // inside the same final watermark box so the configured visual size does
    // not change with output resolution.
    final shadowAlpha = img.Image(
      width: scaled.width,
      height: scaled.height,
      numChannels: 4,
    );
    shadowAlpha.clear(img.ColorRgba8(0, 0, 0, 0));

    for (var yy = 0; yy < scaled.height; yy++) {
      for (var xx = 0; xx < scaled.width; xx++) {
        final a = scaled.getPixel(xx, yy).a;
        if (a == 0) continue;
        final sa = math.min(210, (a * 0.82).round());
        shadowAlpha.setPixelRgba(xx, yy, 0, 0, 0, sa);
      }
    }

    // Offset by at most one final pixel. This is an edge separator, not a
    // second outline, so the timestamp remains visually close to the source.
