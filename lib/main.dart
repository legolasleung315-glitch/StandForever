...
void _drawWatermark(
  ...
  // Dynamic low-resolution pixel-glyph reconstruction.
  //
  // The output width is continuous, so there are no fixed 500/600/800
  // renderers. The glyph is first rasterized from the selected BMFont and
  // tightly cropped, then the final-size pixels are reconstructed by MAX
  // alpha pooling rather than averaging them with Linear interpolation.
  // This prevents thin date strokes from becoming half-gray pixels at
  // 500-800 px while keeping the configured visual size proportional.
  final scale = image.width / 1920.0;
  final targetHeight =
      (fontSize * scale).round().clamp(8, 160).toInt();
  ...
  final bool lowRes = targetHeight <= 32;

  img.Image scaled;

  if (lowRes) {
    // Build a sufficiently dense source raster, then reduce it to the exact
    // final dimensions using max-alpha pooling. Unlike Linear/average
    // sampling, a one-pixel source stroke survives when it lands between
    // final pixels.
    final workScale = targetHeight <= 18 ? 8 : 4;
    final workWidth = math.max(1, targetWidth * workScale);
    final workHeight = math.max(1, targetHeight * workScale);
    ...
    for (var yy = 0; yy < targetHeight; yy++) {
      ...
      final sy1 = math.max(
        sy0 + 1,
        ((yy + 1) * workHeight / targetHeight).ceil(),
      );
      ...
      for (var xx = 0; xx < targetWidth; xx++) {
        ...
        final sx1 = math.max(
          sx0 + 1,
          ((xx + 1) * workWidth / targetWidth).ceil(),
        );
        ...
        for (var sy = sy0; sy < sy1 && sy < workHeight; sy++) {
          for (var sx = sx0; sx < sx1 && sx < workWidth; sx++) {
            final p = dense.getPixel(sx, sy);
            if (p.a > bestA) {
              bestA = p.a;
              bestR = p.r;
              bestG = p.g;
        ...
        if (bestA > 0) {
          // Keep the user's opacity while ensuring the selected glyph pixel
          // is not weakened by averaging.
          final a = bestA.clamp(0, 255);
          scaled.setPixelRgba(xx, yy, bestR, bestG, bestB, a);
        }
      }
    }

    // One-pixel edge separator only when there is room. It is applied to the
    // final pixel mask, so it does not change the continuous size calculation.
    if (targetHeight <= 18 && targetWidth > 2) {
      final edge = img.Image(
        width: targetWidth,
        height: targetHeight,
        numChannels: 4,
      );
      edge.clear(img.ColorRgba8(0, 0, 0, 0));
        ...
        for (var xx = 0; xx < targetWidth; xx++) {
          ...
          for (final p in <List<int>>[
            ...
            if (ex >= 0 &&
                ...
                ex < targetWidth &&
                ey < targetHeight &&
                edge.getPixel(ex, ey).a < (a * 0.65).round()) {
              edge.setPixelRgba(
                ex,
                ey,
                0,
