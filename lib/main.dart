...
void _drawWatermark(
  ...
  // Small output images (for example 500 px wide) can make the bitmap
  // watermark look soft because the previous linear resize blends the
  // glyph pixels together. Keep the exact same target size/position, but
  // use nearest-neighbour sampling for small watermarks so the character
  // strokes stay crisp. Larger watermarks keep the smoother linear mode.
  final scaled = img.copyResize(
    overlay,
    width: targetWidth,
    height: targetHeight,
    interpolation: targetHeight <= 24
        ? img.Interpolation.nearest
        : img.Interpolation.linear,
  );

  // At very small sizes, reinforce partially transparent edge pixels a
  // little. This improves legibility without changing the output image
  // dimensions or the configured watermark size.
  if (targetHeight <= 16) {
    for (var y = 0; y < scaled.height; y++) {
      for (var x = 0; x < scaled.width; x++) {
        final p = scaled.getPixel(x, y);
        final boostedAlpha = (p.a * 1.12).round().clamp(0, 255);
        p.a = boostedAlpha;
      }
    }
  }
