...
class _PhotoProcessorPageState extends State<PhotoProcessorPage> {
  ...
  Future<void> _loadSavedSettings() async {
    ...
    setState(() {
      ...
      _maxWidth = prefs.getInt('maxWidth') ?? _maxWidth;
  ...
  void _applyTodayWorkMode() {
    ...
    setState(() {
      ...
      _extraWatermarkText = '';
      _renameMode = 0;
      _startDateTime = DateTime(now.year, now.month, now.day, 9);
      _endDateTime = DateTime(now.year, now.month, now.day, 11, 30);
      _createWorkFolder = true;
      _statusMessage = '已套用「今日工作模式」';
    });
  }
  ...
  Future<void> _previewFirstImage() async {
    ...
    try {
      ...
      final config = _buildProcessConfig(target);
  ...
  Map<String, dynamic> _buildProcessConfig(DateTime target) {
...
Future<Uint8List> _processImageInIsolate(
  ...
  final maxWidth = (config['maxWidth'] as int).clamp(200, 4000).toInt();
...
void _drawWatermark(
  ...
  // The image package supplies 14/24/48 px bitmap fonts. Render at 48
  // then resize the transparent text layer to the requested pixel height.
  ...
  final rawWidth = textWidth + 6;
  final rawHeight = textHeight + 6;
  ...
  // Draw a subtle dark shadow first so the date remains readable on both
  // bright and dark photos, then draw the requested color on top.
  ...
  final targetHeight =
      fontSize.round().clamp(14, 160).toInt();
  final targetWidth =
