...
const MethodChannel _mediaGalleryChannel =
    ...
    try {
      for (int i = 0; i < total; i++) {
        ...
        try {
          ...
          // Notify Android's media scanner so gallery apps can discover the
          // newly created JPG. The original output path is unchanged.
          try {
            await _mediaGalleryChannel.invokeMethod<void>(
              'scanFile',
              <String, dynamic>{'path': safePath},
            );
          } catch (e) {
            // Export itself succeeded; gallery indexing failure must not make
            // the image job fail.
            _errors.add(
              '${picked.name}：圖片已輸出，但無法通知相簿更新：$e',
            );
          }
...
Future<Uint8List> _processImageInIsolate(
  ...
  if (filterMode == 1) {
    ...
    // Note 2: deliberately flatter, less colourful and more grey than the
    // previous version, matching the supplied reference direction.
    image = img.adjustColor(
      image,
      contrast: 0.86,
      saturation: 0.68,
    ...
    image = img.colorOffset(image, red: -1, green: 0, blue: 2);

    // Mix in a controlled amount of grayscale without removing all colour.
    for (int y = 0; y < image.height; y++) {
      for (int x = 0; x < image.width; x++) {
        ...
        const grayMix = 0.32;
        pixel.r = (pixel.r * (1 - grayMix) + luminance * grayMix)
            .round()
            .clamp(0, 255);
        pixel.g = (pixel.g * (1 - grayMix) + luminance * grayMix)
            .round()
            .clamp(0, 255);
        pixel.b = (pixel.b * (1 - grayMix) + luminance * grayMix)
            .round()
            .clamp(0, 255);
      }
    }

    // Keep the small softening from the previous Note 2 treatment.
...
void _applyGrain(img.Image image, int amount) {
  // Adjustable film-like grain. The slider is intentionally nonlinear enough
  // to make the change visible even on 500-800 px output images.
  // 0 = off; 100 = strongest setting.
  final random = math.Random(20260823);
  final strength = math.max(1, (amount * 0.18).round());
  final coverage = (0.55 + amount * 0.004).clamp(0.55, 0.95);
  ...
  for (int y = 0; y < image.height; y++) {
    for (int x = 0; x < image.width; x++) {
      ...
      // Mostly monochrome luminance grain, like consumer-camera film noise.
...
void _drawWatermark(
  ...
  // Render the bitmap glyph through a higher-resolution intermediate. The
  // source font is still a bitmap font, but this avoids throwing away its
  // available edge pixels in a single tiny resize.
  final bool tiny = targetHeight <= 18;
  final int workScale = tiny ? 4 : (targetHeight <= 32 ? 3 : 2);
  ...
  // Tiny text is most vulnerable to JPEG quantisation. Increase alpha
  // separation only at the very small end; the dimensions and colour stay
  // unchanged.
  if (tiny) {
    for (var yy = 0; yy < scaled.height; yy++) {
      for (var xx = 0; xx < scaled.width; xx++) {
        ...
        final a = p.a / 255.0;
        final boosted = ((a - 0.42) * 1.45 + 0.42).clamp(0.0, 1.0);
