    ...
    try {
      for (int i = 0; i < total; i++) {
        ...
        try {
          ...
          // Also register/save the generated JPG in Android's media gallery.
          // The original selected output folder is still preserved; this
          // simply makes the new photo visible to Gallery/Photos apps.
          try {
...
Future<Uint8List> _processImageInIsolate(
  ...
  // Safe film-style colour modes. Mode 0 stays untouched.
  // Mode 2 (Note2) deliberately moves toward the softer, greyer look:
  // lower contrast + lower saturation + stronger grayscale contribution.
  if (filterMode == 1) {
    _applyDigitalIxusLook(image);
  } else if (filterMode == 2) {
    _applyNote2Look(image);
  }
...
void _applyDigitalIxusLook(img.Image image) {
  // Mild warm/contrast treatment retained for mode 1.
  for (int y = 0; y < image.height; y++) {
    for (int x = 0; x < image.width; x++) {
      final p = image.getPixel(x, y);
      final lum = 0.299 * p.r + 0.587 * p.g + 0.114 * p.b;
      final contrast = (lum - 128.0) * 1.04 + 128.0;
      p.r = (p.r * 0.97 + contrast * 0.03 + 3).clamp(0, 255).round();
      p.g = (p.g * 0.99 + contrast * 0.01).clamp(0, 255).round();
      p.b = (p.b - 2).clamp(0, 255).round();
    }
  }
}

void _applyNote2Look(img.Image image) {
  // Note2 reference direction: softer contrast, noticeably lower saturation,
  // and a visible but still photographic grayscale contribution.
  const contrast = 0.84;
  ...
  for (int y = 0; y < image.height; y++) {
    for (int x = 0; x < image.width; x++) {
      ...
      final softR = (128.0 + (r - 128.0) * contrast) * saturation + lum * (1.0 - saturation);
      final softG = (128.0 + (g - 128.0) * contrast) * saturation + lum * (1.0 - saturation);
      final softB = (128.0 + (b - 128.0) * contrast) * saturation + lum * (1.0 - saturation);

      final outR = softR * (1.0 - grayMix) + lum * grayMix;
      final outG = softG * (1.0 - grayMix) + lum * grayMix;
      final outB = softB * (1.0 - grayMix) + lum * grayMix;

      p.r = outR.clamp(0.0, 255.0).round();
      p.g = outG.clamp(0.0, 255.0).round();
      p.b = outB.clamp(0.0, 255.0).round();
    }
  }
}
