...
void _drawWatermark(
  ...
  // Keep the watermark at the same *relative* size on different output
  // resolutions. 32 px is the reference size at a 1920 px wide image.
  // Without this scaling, a 32 px watermark looks much larger on a
  // 1000 px image and much smaller on a 4000 px image.
  ...
  // High-quality bitmap-font pipeline:
  //
  // The final watermark size is NOT changed. We first enlarge the already
  // rasterized glyph mask, then reduce it in a controlled pass. This avoids
  // the overly soft result produced by doing one direct linear resize from
  // the original bitmap font to a tiny 500 px output.
  //
  // Bitmap fonts cannot create new glyph detail that is not present in the
  // source atlas, so the goal here is controlled edge sampling and contrast,
  // not artificial enlargement.
  ...
  final enlarged = img.copyResize(
    ...
    width: workWidth,
    height: workHeight,
    // Preserve the bitmap glyph pixels while creating the high-resolution
    // intermediate mask.
    interpolation: img.Interpolation.nearest,
  );

  // Reduce the high-resolution mask with a smooth filter. This produces
  // cleaner anti-aliased edges than a single direct resize, while retaining
  // the same final pixel dimensions.
  ...
  // Small text needs slightly stronger alpha contrast. Do not increase the
  // watermark dimensions; only make weak edge pixels less likely to disappear
  // into the photo after JPEG encoding.
  if (tiny) {
    for (var y = 0; y < scaled.height; y++) {
      for (var x = 0; x < scaled.width; x++) {
        final p = scaled.getPixel(x, y);
        final a = p.a.toDouble();
        // Gentle S-curve around the middle alpha range.
        final normalized = a / 255.0;
        final contrasted =
            ((normalized - 0.5) * 1.28 + 0.5).clamp(0.0, 1.0);
        p.a = (contrasted * 255.0).round();
      }
    }
  }

  // For medium/small output sizes, retain the cleaner high-resolution
  // reduction but avoid extra sharpening that could make larger text harsh.
