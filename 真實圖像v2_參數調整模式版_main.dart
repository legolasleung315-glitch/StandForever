        ...
        return AlertDialog(
          ...
          content: TextField(
            ...
            decoration: InputDecoration(
              ...
              hintText: '${_formatEditValue(min)}–${_formatEditValue(max)}',
            ),
            onSubmitted: (_) {
      ...
      body: ListView(
        ...
        children: [
          ...
          if (_adjustmentMode == 0) ...[
            ...
            _precisionControl(
              title: '字體大小', value: _fontSize.round(), min: 14, max: 160,
              step: 1, unit: ' px',
              disabled: _isProcessing,
              onChanged: (v) => _setStateAndSave(() => _fontSize = v.toDouble()),
            ),
            _precisionControl(
              title: '邊距', value: _margin.round(), min: 8, max: 80,
              step: 1, unit: ' px',
              disabled: _isProcessing,
              onChanged: (v) => _setStateAndSave(() => _margin = v.toDouble()),
            ),
            _precisionControl(
              title: '透明度', value: (_opacity * 100).round(), min: 20, max: 100,
              step: 1, unit: '%',
              disabled: _isProcessing,
              onChanged: (v) => _setStateAndSave(() => _opacity = v.toDouble() / 100),
            ),
          ],
          TextFormField(
          ...
          if (_adjustmentMode == 0) ...[
            ...
            Slider(
              value: _grainAmount.toDouble(), min: 0, max: 100, divisions: 20,
              label: _grainAmount == 0 ? '關閉' : '$_grainAmount%',
              onChanged: _isProcessing ? null : (v) => _setStateAndSave(() => _grainAmount = v.round()),
            ),
            Text(_noiseAmount == 0 ? '噪點：關閉' : '噪點：$_noiseAmount%'),
            Slider(
              value: _noiseAmount.toDouble(), min: 0, max: 100, divisions: 20,
              label: _noiseAmount == 0 ? '關閉' : '$_noiseAmount%',
              onChanged: _isProcessing ? null : (v) => _setStateAndSave(() => _noiseAmount = v.round()),
            ),
          ] else ...[
            _precisionControl(
              title: '顆粒', value: _grainAmount, min: 0, max: 100,
              step: 5, unit: '%', disabled: _isProcessing,
              onChanged: (v) => _setStateAndSave(() => _grainAmount = v.round()),
            ),
            _precisionControl(
              title: '噪點', value: _noiseAmount, min: 0, max: 100,
              step: 5, unit: '%', disabled: _isProcessing,
              onChanged: (v) => _setStateAndSave(() => _noiseAmount = v.round()),
            ),
          ],
          ...
          if (_adjustmentMode == 0) ...[
            ...
            Slider(
              value: _jpegQuality.toDouble(), min: 60, max: 98, divisions: 38,
              onChanged: _isProcessing ? null : (v) => _setStateAndSave(() => _jpegQuality = v.round()),
            ),
            Text('輸出尺寸（寬度）：$_maxWidth px'),
            Slider(
              value: _maxWidth.toDouble().clamp(500, 4000), min: 500, max: 4000, divisions: 70,
              onChanged: _isProcessing ? null : (v) => _setStateAndSave(() => _maxWidth = v.round()),
            ),
          ] else ...[
            _precisionControl(
              title: 'JPG 品質', value: _jpegQuality, min: 60, max: 98,
              step: 1, unit: '', disabled: _isProcessing,
              onChanged: (v) => _setStateAndSave(() => _jpegQuality = v.round()),
            ),
            _precisionControl(
              title: '輸出尺寸', value: _maxWidth, min: 500, max: 4000,
              step: 50, unit: ' px', disabled: _isProcessing,
              onChanged: (v) => _setStateAndSave(() => _maxWidth = v.round()),
            ),
          ],
          SwitchListTile(
            contentPadding: EdgeInsets.zero,
